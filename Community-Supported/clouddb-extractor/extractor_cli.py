...
def _exclusive_args(args, *arg_names, required=True, message=None):
    count_args = 0
...
def _required_arg(args, arg_name, message=None):
...
def _add_arg_with_default(parser, config, default_config_key, help, required, *args, **kwargs):
    default_value = None
...
def _get_int_from_arg(this_str, arg_name, is_required=False):
    if this_str is None:
        if is_required:
            raise IllegalArgumentError("Missing required argument:{}".format(arg_name))
        else:
...
- delete: Delete rows from a published datasource that match a condition and/or
  that match the primary keys in the changeset from a query or table
    ...
    _add_arg_with_default(parser, config, "sample_rows", "Defines the number of rows to use with LIMIT when command=load_sample", False, "--sample_rows")
    ...
    _add_arg_with_default(parser, config, "tableau_env.project", "Target project name", True, "--tableau_project", "-P")
    _add_arg_with_default(parser, config, "tableau_env.datasource", "Target datasource name", True, "--tableau_datasource")
    _add_arg_with_default(parser, config, "tableau_env.username", "Tableau user name", False, "--tableau_username", "-U")
    _add_arg_with_default(parser, config, "tableau_env.token_name", "Personal access token name", False, "--tableau_token_name")
    _add_arg_with_default(parser, config, "tableau_env.token_secretfile", "File containing personal access token secret", False, "--tableau_token_secretfile")
